        ...
        <form method="POST" action="{{ route('admin.email-templates.update', $template->id) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea name="body_html" rows="14" class="w-full px-3 py-2 border border-gray-300 rounded-md font-mono text-xs">{{ old('body_html', $template->body_html) }}</textarea>
                    <p class="mt-1 text-xs text-gray-500">
                        Full HTML email content. Use inline styles where necessary. No emojis.
                    </p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Plain text body</label>
                    <textarea name="body_text" rows="14" class="w-full px-3 py-2 border border-gray-300 rounded-md font-mono text-xs">{{ old('body_text', $template->body_text) }}</textarea>
                    <p class="mt-1 text-xs text-gray-500">
                        Fallback text version for clients that do not render HTML.
                    </p>
                </div>
            </div>
